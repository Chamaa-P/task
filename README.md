...
| Team Member | Student Number | Preferred Email |
| --- | --- | --- |
...
6. Produce a deployment and development workflow that is reproducible for instructors, TAs, and future contributors.

## Technical Stack

| Area | Technologies | Notes |
| --- | --- | --- |
| Frontend | React, Vite, TypeScript, React Router, TanStack Query, Zustand, Socket.IO client | Browser UI for authentication, dashboards, projects, tasks, and calendar views |
| Backend | Node.js, Express, TypeScript, Sequelize, Socket.IO, JWT, bcryptjs, Helmet, Morgan, prom-client | REST API, authentication, real-time events, and metrics |
...
The implemented features satisfy the project goals and cover the course requirements for containerization, persistence, orchestration, monitoring, and advanced functionality.
...
4. Day-based calendar view
   - Due dates are handled as calendar days instead of timestamps.
   - The calendar groups tasks by date and makes overlapping deadlines from multiple projects easy to see.
...
This README does not embed screenshots, so there are no broken image dependencies in the GitHub view. The live deployment link below can be used for direct inspection.
...
This URL was reachable with an HTTP 200 response on March 30, 2026. The course-compliant production deployment uses DigitalOcean Docker Swarm with one manager node, two worker nodes, replicated frontend services, a backend API service, a PostgreSQL service pinned to the manager, and a provider-backed storage mount at `/mnt/postgres-data`.
...
AI tools were used, but only as support. We reviewed the output critically and verified changes with technical checks before treating them as correct.
...
1. Where AI meaningfully contributed
   ...
   - debugging and refactoring for the day-only due date/calendar behavior
...
3. How correctness was verified
   ...
   - manual UI inspection of authentication, project creation, task creation, status updates, and calendar rendering
...
The summary below is based on the repository's Git history as of March 30, 2026.
...
- Adam Pietrewicz
  - Contributed to the backend foundation, including API routing, backend startup wiring, Docker development files, and Compose-based local workflow setup.
  - The Git history includes both `Adam Pietrewicz` and `Adam P` for the same contributor email.
- Patrick Chamaa
...
Overall, the commit history shows that frontend, backend, and infrastructure work were distributed across the team, with responsibilities aligning well to the final system architecture.

## Lessons Learned and Concluding Remarks

This project reinforced that cloud deployment is not just about getting containers to run. The harder and more valuable work was making stateful services reliable, keeping configuration consistent across environments, and verifying that documentation matched the real deployment. We also learned that advanced features such as monitoring, autoscaling, and real-time updates add substantial integration complexity, especially when persistence and security must remain correct at the same time.

In conclusion, TaskCollab met our main goals: it is a stateful, cloud-native application with a clear user-facing workflow, a reproducible local setup, and a course-compliant Docker Swarm deployment on DigitalOcean. Just as importantly, the project gave us practical experience connecting application development with deployment, observability, persistence, and operational verification.
